...
def get_args():
    ...
    parser.add_argument(
        "--inputpath",
        type=str,
        help="Path to the CSV file generated by Step_1_QA_backup.py",
        default="./logs/qa_output.csv"
    )
    parser.add_argument(
        "--model",
        type=str,
...
def main():
    ...
    try:
        results_df = pd.read_csv(args.inputpath, sep="@")
    ...
    try:
        ...
        print("\n--- Average Scores for 'minirag' ---")
        print(minirag_result_df[['faithfulness', 'answer_relevancy', 'context_recall', 'context_precision']].mean())
        # saving the detailed results to a CSV file
        output_csv = "./logs/" + os.path.splitext(os.path.basename(args.inputpath))[0] + f"_minirag_evaluation_{args.model}.csv"
        minirag_result_df.to_csv(output_csv, index=False)
    ...
    try:
        ...
        print("\n--- Average Scores for 'naive' ---")
        print(naive_result_df[['faithfulness', 'answer_relevancy', 'context_recall', 'context_precision']].mean())
        # saving the detailed results to a CSV file
        output_csv = "./logs/" + os.path.splitext(os.path.basename(args.inputpath))[0] + f"_naive_evaluation_{args.model}.csv"
        naive_result_df.to_csv(output_csv, index=False)
        print(f"Detailed results saved to: {output_csv}")
