...
/* Buttons: coral by default (Ask), examples overridden below */
.stButton > button {
  ...
  margin-top:0 !important;  /* keeps vertical alignment with inputs */
}

/* Example chips — light look, wrap nicely, no overflow */
.examples { display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:10px; }
.examples .stButton > button {
  background:#fafafa !important; border:1px solid #e5e7eb !important; color:#374151 !important;
  height:auto; line-height:1.2; white-space:normal; text-align:center;
  border-radius:999px !important; padding:8px 12px;
}
.examples .stButton > button:hover { background:#f3f4f6 !important; }

/* Q/A (plain text; no rounded “pills”) */
.qa { margin-top:10px; }
...
.source-box {
  ...
  white-space:pre-wrap; word-break:break-word;  /* full chunk, no truncation */
}

/* Sidebar history: tidy, wrapped text */
...
# Example chips (light style, responsive grid)
