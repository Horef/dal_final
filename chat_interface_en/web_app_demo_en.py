...
.ask-wrap button{border-radius:12px !important;height:44px;font-weight:600;
  background:#ef4444 !important; border:1px solid #ef4444 !important; color:#fff !important;}
.ask-wrap button:hover{filter:brightness(0.97)}

/* Example chips – responsive */
.examples{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px}
.examples .stButton > button{
  border:1px solid #e5e7eb !important; background:#fafafa !important; color:#374151 !important;
  border-radius:999px !important; padding:8px 12px; font-size:13px; height:auto; line-height:1.2;
  white-space:normal; text-align:center;
}
.examples .stButton > button:hover{background:#f3f4f6 !important}
...
.source-item-title{font-weight:700; margin:6px 0 6px 0;}
.source-box{
...
[data-testid="stSidebar"] .stButton > button{
  border:1px solid #e5e7eb !important; background:#fff !important; color:#111827 !important;
  border-radius:10px !important; padding:8px 10px; height:auto; line-height:1.25;
...
if st.session_state.history:
    for i, turn in enumerate(reversed(st.session_state.history)):
        ...
        st.sidebar.button(
            turn["q"], key=f"h_{idx}",
            on_click=load_from_history, args=(idx,), use_container_width=True
        )
else:
    st.sidebar.caption("No questions yet.")
st.sidebar.markdown('<div class="clear-btn">', unsafe_allow_html=True)
if st.sidebar.button("Clear history", use_container_width=True): st.session_state.history = []
    ...
    if turn["sources"]:
        with st.expander("Sources", expanded=False):
            for i, s in enumerate(turn["sources"], 1):
                st.markdown(f'<div class="source-item-title">{i}. {s["title"]} • score {s["score"]:.2f}</div>', unsafe_allow_html=True)
